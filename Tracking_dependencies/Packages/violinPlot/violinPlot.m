        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    [xHist,yHist,u] = ksdensity(currentData,x,'kernel','normal');
                    % take smaller kernel to avoid over-smoothing
                    if opt.divFactor ~= 1
                        [xHist,yHist] = ksdensity(currentData,x,'kernel','normal','width',u/opt.divFactor);
                    end
                end
        ...
        if opt.showMM == 6
            ...
            for qq = 1:3
                ...
                yLoc =  repmat(...
                    ...
                    [4 1]);
                if sum(yLoc == 0)
                    % in case of median being exactly on bin edge
                     yLoc =  repmat(...
                    any(yArray>md(iData,qq,1)-.0000001,1) & any(yArray<=md(iData,qq,1)+.0000001,1),...
                    [4 1]);
                end
...
if opt.showMM
    ...
    switch opt.xyOri
        case 'normal'
            ...
            if opt.showMM == 6
                mdh(1,:) = plot(ah,squeeze(md(:,1,2:3))',repmat(md(:,1,1)',2,1),'color','k','lineWidth',1,'lineStyle','-');
