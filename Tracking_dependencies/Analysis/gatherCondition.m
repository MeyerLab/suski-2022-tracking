function [S] = gatherCondition(settings)
            ...
            for col = colMat
                for site = siteMat
                    ...
                    if exist(fullfile(settings.dataDir,[settings.liveLabel,shot,'.mat'])) && (~settings.IFoption || exist(fullfile(settings.dataDir,[settings.IFlabel,shot,'.mat'])))
                            loadLive = load(fullfile(settings.dataDir,['traceData_',shot,'.mat']),'tracedata','genealogy','jitters');
                            cellStore{idx,1} = loadLive.tracedata;
                            ...
                            cellStore{idx,6} = repmat(string(shot),size(loadLive.genealogy,1),1);
                            idx = idx + 1;
                    end
                end
            end
