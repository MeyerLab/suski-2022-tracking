function [tracedata,curdatatracked,tracking,nuc_label, mitosis_flag]=adaptivetrack_NR(tf,lastgoodframe,curframe,tracedata,curdata,tracking,nuc_raw,nuc_label,jitter,trackparams,debugpackage)
nucr=trackparams{1};
...
%     if abs(xprev(p)-1394)<20 && abs(yprev(p)-2517)<2 && tf==2
%         keyboard;
%     end
    ...
    if splitcheck
        ...
        if tracking(cell1,1)==tracking(cell2,1)  %sisters NR:maybe change this to prevent cells which divided a long time before merging from being randomly assigned (if cells are within jump window, try to assign based on position)
