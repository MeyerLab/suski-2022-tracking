function [ names ] = output_names_puncta_number( sigs,localbg,ring,punctacalc, punctaThresh, varThresh)
...
if punctacalc
    ...
    for i=1:size(punctaThresh,2)
        length = length+1;
        names(1:2,length) = {length; ['Filter Masked num ' num2str(punctaThresh(i))]};
    end
    ...
    for i=1:size(varThresh,2)
        length = length+1;
        names(1:2,length) = {length; ['Variance Masked num ' num2str(varThresh(i))]};
    end 
