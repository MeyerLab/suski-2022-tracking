function [firstgoodindex,blurthreshhigh,blurthreshlow,numthresh,badframes,height,width]=timelapsesetup_4(rawdir,name1,frames,nucr,blobthreshold,debrisarea,badframes,maskwrite)
%%% determine median cell size for blur detection %%%%%%%%%%%%%%%%%%%%%%%%%
nuc_area=zeros(3,1); numcells=zeros(3,1);
for i=1:3
    raw1=log(single(imread([rawdir,name1,'_', num2str(frames(i)),'.tif'])));
