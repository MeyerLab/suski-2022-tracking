...
%%% Paths
s.experiment_name='C192-live';
s.image_drive = 'E:\4TB11\Data\';
%s.savepath=['F:\Data\D-Replication Initiation\',s.experiment_name,'\Data\'];
s.savepath=['F:\Data\C-Cdt1\C192-live\Data\'];
...
s.endFrame=84;  %Frame to stop analyzing
...
if ~exist([s.savepath,'tracedata_',shot,'.mat'],'file')
    ...
    for i = firstgoodindex:totalframes
        ...
        if i>firstgoodindex+1 && (numcells<numthresh || mednuc>blurthreshhigh || mednuc<blurthreshlow)
            ...
            extractmask = bwmorph(nuc_mask,'remove');
            if s.maskwrite
                imwrite(uint16(extractmask),[maskdir,shot,'_',s.maskname,num2str(f),'.tif']);
            end
            continue;
        end
