...
for i = 1:length(names)
    for j = 1:length(names{i})
        ...
        if i==1
            whole_mask = logical(nuc_mask{1});
            nuc_mask{1} = bwareaopen(nuc_mask{1},debrisarea);
            
            %%% adaptive bridging settings %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [bounds,label_mask]=bwboundaries(nuc_mask{1},'noholes');
            mask_props = regionprops(label_mask,'Area','Solidity');
            sizes = [mask_props(:).Area];
            solid = [mask_props(:).Solidity];
            medSize = prctile(sizes,50);
            gate = sizes >= medSize | solid < solidity;
            
            nuc_mask{1} = segmentdeflections_gated(nuc_mask{1},nucr,bounds, gate);
            nuc_mask{1} = bwareaopen(nuc_mask{i},debrisarea);
            
            clean_mask = excludelargeandwarped_3(nuc_mask{1},boulderarea,solidity);
            clean_mask = imclearborder(clean_mask);
            exclude_mask = nuc_mask{1} - clean_mask;
            nuc_mask{1} = clean_mask;
            
            %% Check for bad frame %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            IF_foreground = sum(nuc_mask{1}(:))/length(nuc_mask{1}(:));
    ...
    for j = 1:length(names{i})
        if settings.punctacalc{i}(j)
            ...
            for t = 1:length(settings.punctaThresh{i}{j})
                punctaMask{i}{j}{t} = punctaFilt > settings.punctaThresh{i}{j}(t);
...
%%% Clear unnecessary variables
clear nuc_mask_aligned exclude_mask_aligned nanmask_aligned real unblurred punctaMask punctaIntensity nanmask_aligned nanmaskcyto_aligned
...
for i = 1:length(names)
    for j = 1:length(names{i})
        ...
        for n = 1:numlivecells
            ...
            if settings.punctacalc{i}(j)
                for t = 1:length(settings.punctaThresh{i}{j})
                    punctaArea{i}{j}(cc,t) = sum(punctaMask_padded{i}{j}{t}(mask_info(cc).PixelIdxList));
                    punctaIntensity{i}{j}(cc,t) = sum(punctaIntensity_padded{i}{j}{t}(mask_info(cc).PixelIdxList));
                    numPuncta = length(unique(puncta_label{i}{j}{t}(mask_info(cc).PixelIdxList))) - 1;
