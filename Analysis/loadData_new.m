...
%%% Conditions %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
settings.conditions = {
        'DMSO',2:4,2:11,1,[0 0 0],[0 10]; %1    
        'TAK',5:7,2:11,1,[0 0 0],[0 10]; %1  

    };

%%% Paths %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
settings.dataDir = 'F:\Data\D-Replication Initiation\D127-live\Data';
...
settings.saveName = 'D127_data.mat';
settings.saveDir = 'F:\Data\D-Replication Initiation\D127-live\Plot';
%%% Analysis Options %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
settings.daughterGate = 1;  %0:no gating 1:daughters 2:no daughters
settings.type = 'cycling';   %'release', 'cycling' or 'hybrid
settings.minTraceFrac = .75;
settings.nuc = 'iRFP';
settings.cdk = 'CFP';
settings.apc = 'YFP';
settings.crl = 'RFP';
...
%     badNoise = gatenoisy(S(i).massNorm, S(i).traceStats, settings.daughterOption, strcmp(settings.type,'release'), noiseThresh, 4,4);
    ...
    if ~isempty(settings.cdk)
        S(i).cdkNuc = S(i).traceData(:,:,ismember(names,[settings.cdk '_mean']));
        S(i).cdkCyt = S(i).traceData(:,:,ismember(names,[settings.cdk '_cyto ring']));
        S(i).cdkNuc = S(i).cdkNuc - prctile(S(i).cdkCyt,1,1);
        S(i).cdkCyt = S(i).cdkCyt - prctile(S(i).cdkCyt,1,1);
        ...
        S(i) = gateout_all(S(i),~badTracesCdk & S(i).isDaughter);
    end
...
%         S(i).apcNuc = fillTraceVals(S(i).apcNuc, S(i).traceStats, 5);
        ...
        if settings.poiApc            
            ...
            [S(i).POI(subset,2), badTracesAPC(subset)] = findAPCInact(S(i).apcArea(subset,:), S(i).traceStats(subset,:), ...
                struct('postBuffer',10,'smooth',5,'cycling',0,...
                'buff',1,'thresh',.4,'preBuffer',1,'lowThresh',30, 'increase',10*.4,'trunc',100, ...
                'medfilt',0,'early', 30,'debug',0));
            %%% Divided cells
            subset = find(S(i).isDaughter);
            traces = S(i).apcNuc;
            [S(i).POI(subset,2), badTracesAPC(subset)] = findAPCInact(S(i).apcAreaNormM(subset,:), S(i).traceStats(subset,:), ...
                struct('postBuffer',10,'smooth',7,'cycling',1,...
                        ...
                        'trunc',.5, 'medfilt',0,'early', 5,'debug',0));
...
%         S(i).crlNuc = fillTraceVals(S(i).crlNuc, S(i).traceStats, 5);
        ...
        if settings.poiCrl          
            ...
            S(i).POI(subset,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea(subset,:),...
                ...
                'firstD',0,'secD',-.005,'decrease', .1, 'minSlope', -.01, 'early', 20,'falseCall', .8, 'cytoCheck', 0,'minExp',75,...
                'trunk',.4,'debug',0));
            %%% Divided cells
            subset = find(S(i).isDaughter);
            S(i).POI(subset,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea(subset,:),...
                ...
                'firstD',0,'secD',-.005,'decrease', .05, 'minSlope', -.01, 'early', 3,'falseCall', .8, 'cytoCheck', 0,'minExp',100,...
                'trunk',.4,'debug',0));
            ...
            if settings.poiG2
                [S(i).POI(:,5), badTracesCRL] = findCRLInact(S(i).crlNormAct, S(i).traceStats, S(i).POI(:,4), ...
                    ...
                    'increase',.005*10,'trunc',.5, 'medfilt',0,'early', 15,'debug',0));
            end
        end
